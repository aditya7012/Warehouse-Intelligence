...
if product == "All Products":
    ...
    col1.metric("📦 Orders Tomorrow",    f"{store_prediction:,}")
    col2.metric("🛵 Partners Needed",    f"{riders_needed}")
    col3.metric("📊 Avg Daily Orders",   f"{avg_daily:,}")
    col4.metric("🗓️ Last 7 Days Orders", f"{total_orders_7d:,}")
...
# ═══════════════════════════════════════════
    ...
    # Aggregate duplicates first (same date can appear multiple times per SKU)
    hist_agg = product_data.groupby("date")["orders"].sum().reset_index()
    hist_agg = hist_agg.sort_values("date")

    future_dates = pd.date_range(hist_agg["date"].max() + pd.Timedelta(days=1), periods=3)
    forecast_rows = pd.DataFrame({
        "date":     future_dates,
        "Actual":   [None, None, None],
        "Forecast": predictions.astype(int).tolist(),
    })
    hist_rows = hist_agg.rename(columns={"orders": "Actual"}).copy()
    hist_rows["Forecast"] = None

    chart_df = pd.concat([hist_rows, forecast_rows], ignore_index=True)
